...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        currentFactorsList = normalize(joinFactors(currentFactorsList))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for value in self.values():
            TotalSum += value 
        r = random.random() * TotalSum

        step = 0
        for key, value in self.items():
            if step <= r and r < step + value:
                return key
            step += value
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for Pos in self.allPositions:
            newPosDict = self.getPositionDistribution(gameState, Pos)
            for oldPos in self.allPositions:
                probdict[oldPos] += self.beliefs[Pos] * newPosDict[oldPos]
        
        for oldPos in self.allPositions:
            self.beliefs[oldPos] = probdict[oldPos]
